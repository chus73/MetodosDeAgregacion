...
Tiene la ventaja de que sólo usa la matriz de distancia, por lo que es independiente de las observaciones.
...
#### Clusterización mediante Nearest Neighbors
Recordemos que hemos aplicado el métoo min-max normalization para el reescalado de los valores del dataset.

Para 


Primero de todo vamos a crear los datasets de training y test. Tenemos un dataset con 200 valores, cogeremos un 10% de ellos para test.
...
df.predict <- knn(train = df.norm.train, test = df.norm.test, )
```
